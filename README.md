...
Create a `.env` file in the project root (or update the existing one) with your Voximplant CI credentials and application settings:
...
> 💡 Tip: Make sure the `.env` file is **not committed to version control**, as it contains sensitive credentials.
...
The integration with Grok requires an **API key** and **system instructions** for the LLM. These settings are defined in the module `modules/grok_config.voxengine.js`.

#### Example configuration:

```javascript
const GROK_API_KEY = 'YOUR_GROK_API_KEY';          // Insert your Grok API key
const GROK_INSTRUCTIONS = 'YOUR_INSTRUCTION_FOR_LLM'; // Instructions for the LLM (system prompt)
```

Notes

- `GROK_API_KEY` — your personal API key from Grok (xAI).
- `GROK_INSTRUCTIONS` — the system prompt or instructions you want the LLM to follow during the call.
- These values are used in both **inbound** and **outbound** scenarios.

> 💡 Tip: Keep your API key private. Do not commit `grok_config.voxengine.js` with real keys to a public repository.

### Set Voximplant Phone Number

The project uses a rented Voximplant number for both **incoming** and **outgoing** calls. You need to configure it in two places:

**Outbound calls**
    - Open `scenarios/outbound_handler.voxengine.js`
    - Set the rented number as the caller ID:

```javascript
const voxNum = 'YOUR_RENTED_PHONE_NUMBER'; // Replace this with your rented number
```
...
│
├── modules/
│   ├── grok_config.voxengine.js          # Grok configuration
...
- `modules/grok_config.voxengine.js` — stores Grok API key and instructions for the LLM
